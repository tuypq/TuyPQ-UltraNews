<ul {!! $options !!}>
    @foreach ($menu_nodes as $key => $row)
        <li class="@if ($row->child) nav-dropdown-item @else nav-item @endif  @if ($row->has_child) nav-dropdown group relative @endif @if ($row->css_class) {{ $row->css_class }} @endif ">
            <a class="@if ($row->child) nav-link  @else nav-dropdown-link @endif @if ($row->active) active @endif" href="{{ url($row->url) }}" @if ($row->target !== '_self') target="{{ $row->target }}" @endif>
