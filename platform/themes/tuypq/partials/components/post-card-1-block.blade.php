<div class="mb-8 md:col-6 lg:col-4">
    <div class="card">
      <img
        ...
        width="335"
        height="210"
        src="{{ RvMedia::getImageUrl($post->image, !empty($imageType) ? $imageType : 'medium_large', false, RvMedia::getDefaultImage()) }}"
        alt="{{ $post->name }}"
      />
      <div class="card-content">
          ...
          @if ($category)
        <div class="card-tags">
          <a class="tag" href="{{ $category->url }}">{{ $category->name }}</a>
        </div>
        @endif
        ...
        <div class="card-footer mt-6 flex space-x-4">
          <span class="inline-flex items-center text-xs text-[#666]">
            ...
            {{ $post->created_at->format(post_date_format(false)) }}
          </span>
          <span class="inline-flex items-center text-xs text-[#666]">
            <svg
              class="mr-1.5"
